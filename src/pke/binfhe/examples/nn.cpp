...
int main(int argc, char **argv)
{
    ...
    // Timings
    clock_t bs_begin, bs_end, net_begin, net_end, begin, end;
    ...
    for (int img = 0;(img< n_images); /*img*/ ){
        ...
        for (int i = 0; i < num_neurons_current_layer_in; ++i)
        {
            ...
            if (noisyLWE)
            {
                ...
                auto ct = cc.Encrypt(sk, (pixel+p) % p, p, FRESH);
                enc_imgae_1.push_back(ct);
            }
            else
            {
                //! Encrypt message without noise
                cc.TraivlEncrypt((pixel+p) % p, p);
            }
        }
        ...
        for (l=0; l<num_wire_layers - 1 ; ++l)     // Note: num_wire_layers - 1 iterations; last one is special. Access weights from level l to l+1.
        {
            ...
            for (int j=0; j<num_neurons_current_layer_out; ++j)
            {
                ...
                auto ct = cc.TraivlEncrypt((w0 + p) % p, p);
        ...
        if(test_BF)
        {
            for(int i=0;i<p;i++)
            {
                LWECiphertext temp = cc.Encrypt(sk, i, p, FRESH);
                auto ct_sign = cc.MyEvalSigndFunc(temp, p);
                LWEPlaintext tempp;
                cc.Decrypt(sk, ct_sign, &tempp, p);
                cout<<"input and output after BF are "<<i<<"    "<<tempp<<endl;
            }
        }
        ...
        for (int j=0; j<num_neurons_current_layer_out; ++j)
        {
            ...
            auto ct_sign = cc.MyEvalSigndFunc(multi_sum_1[j], p);
            ...
            if(test_BF && false){
                ...
                if (temp*multi_sum_clear[j] <= 0 || true){
                    cout<<"True value before bootstrapping is "<<multi_sum_clear[j]<<endl;
                    cout<<"ciphertext value before bootstrapping is "<<temp1<<endl;
                    cout<<"ciphertext value after bootstrapping is "<<temp<<endl;
                } 
            }
        ...
        cout<<"the real label is "<<label<<endl;
        cout<<"the recogination without fhe is "<< class_clear<<endl;
        cout<<"the recogination using fhe is "<<class_enc<<endl;
        cout<<"-----------------------------------------------------------------------------------------"<<endl;        
